...
data <- read.csv(file = "Re-evaluation_Analysis/GA_r_analysis.txt", header = TRUE, sep = " ", skip = 14)
dataPlot <- data.frame()
...
png("Re-evaluation_Analysis/GA_r_analysis.png", width = 1920, height = 1200, units = "px")
g <- NA
g <- qplot(x = N_RE_EVALUATIONS, y = N_FITNESS_EVALUATIONS, data = dataPlot, geom="boxplot") 
#g <- g + geom_vline(xintercept = 5, colour="grey50", linetype = "longdash", position = "identity")
#g <- g + geom_text(aes(x=5.5, y=3250000, label="min == 8", group=NULL), size = 5, color = "grey50", parse = T)
g <- g + scale_y_log10(label = scientific_10)
g <- g + theme(plot.margin = unit(c(1,3,1, 1), "cm"))
g <- g + xlab("\n# re-evaluations\n")
g <- g + ylab("\n# fitness evaluations\n")
g <- g + ggtitle("\nRe-evaluation analysis for the (mu + 1)-GA\nsolving binary OneMax with added posterior noise\n(mu = 10, variance = 100, seed = 1)\n")
