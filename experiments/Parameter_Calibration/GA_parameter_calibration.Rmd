---
title: "GA Parameter Calibration"
...
```{r, echo=FALSE}
sample <- NA
data <- read.csv(file = "GA_parameter_calibration.txt", header = TRUE, sep = " ", skip = 14)
data$MU <- as.factor(data$MU)
i <- NA
header <- "N_FITNESS EVALUATIONS with MU = "
for (i in unique(data$MU)){
  sample <- data$N_FITNESS_EVALUATIONS[data$MU == i]
